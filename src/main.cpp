...
#if defined(NDEBUG)
# error "SanchoCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbe09d195f2d09127aae03ec837ce338d27e75c89ce9e75ea7a116c771449bb57");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SanchoCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SanchoCoin Signed Message:\n";
...
{
    // SanchoCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 84000000); // SanchoCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // SanchoCoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // SanchoCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SanchoCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SanchoCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb76165bb852370f1fe1ae788023ed6494a904d7d2af13c7139a21dab7d84a343");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Covid-19 Omicron variant continues to spread arround the world at 27/Dez/2021";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0434145aedca698026b737cd1afa1c698c12fbacbf01cb79b7faff75fb4cb4ec5c2999a8546fcae7bb5789b612b453f6e9c280feba3576913c5f541fbbce78e31e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1640619651;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084926571;

        if (fTestNet)
        {
            block.nTime    = 1640619630;
            block.nNonce   = 388872482;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4ba257a5be77668a01cc47c591042c2a2162238f9a58d5cc25fe137385441eb8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc4, 0xb4, 0xde }; // SanchoCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SanchoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SanchoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SanchoCoinMiner(CWallet *pwallet)
{
    printf("SanchoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sanchocoin-miner");
    ...
    try { loop {
        ...
        printf("Running SanchoCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SanchoCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SanchoCoinMiner, pwallet));
}
